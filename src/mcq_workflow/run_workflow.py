...
def run_sync(cmd: str) -> int:
    """Run a shell command synchronously, propagating its own stdout/stderr."""
    print(f"Running: {cmd}")
    # Start child in its own process group so we can signal it
    proc = subprocess.Popen(
        cmd,
        shell=True,
        preexec_fn=os.setsid,   # Unix: start new session so pgid = pid
    )
    try:
        # Let the child draw its own output (pbar, logs)
        proc.wait()
    except KeyboardInterrupt:
        # Forward SIGINT to the whole group (child + threads)
...
def main() -> None:
    try:
        ...
        if not (project_root / "secrets.yml").exists():
            print(
                "Please create secrets.yml. If using Argo models, populate with:\n"
        ...
        if start_step <= 3:
            ...
            if n_value:
                ...
                if run_sync(
                    f"python -m common.select_mcqs_at_random -i MCQ-combined.json "
